...
const AppDatePicker = ({ label, name, value, onChange, required, dropdownPosition = 'bottom', allowFutureDates = false }) => {
  ...
  const handleYearChange = e => {
    ...
    setViewDate(prev => new Date(newYear, prev.getMonth()))
  }
    ...
    <div className='datepicker-wrapper' ref={ref}>
      ...
      <div className='date-input-wrapper'>
        <input
          ...
          required={required}
          className={`form-input date-input-with-icon${showCalendar && dropdownPosition === 'left' ? ' has-datepicker-left' : ''}${
            showCalendar && dropdownPosition === 'right' ? ' has-datepicker-right' : ''
          }`}
          autoComplete='off'
          readOnly
        />
        <div className='calendar-icon' onClick={handleInputClick}>
          <svg width='20' height='20' viewBox='0 0 20 20' fill='currentColor' className='calendar-svg'>
            <path
              fillRule='evenodd'
